...
module Configuration:
  {
    ...
    let make:
      (~bundlePolicy: string=?, ~certificates: array(PWA_RTCCertificate.t)=?,
      ~iceCandidatePoolSize: int=?, ~iceServers: array(PWA_RTCIceServer.t)=?,
      ~iceTransportPolicy: string=?, ~peerIdentity: string=?,
      ~rtcpMuxPolicy: string=?, unit) => t;
...
let idpassertionerror: PWA_EventType.t(t, {. "idp": string, "loginUrl": Js.Nullable.t(string), "protocol": string});

let idpvalidationerror: PWA_EventType.t(t, {. "idp": string, "loginUrl": Js.Nullable.t(string), "protocol": string});
...
let addTrack: (t, PWA_MediaStreamTrack.t, PWA_MediaStream.t) => PWA_RTCRtpSender.t;
