...
/* TODO optional? */
[@bs.send] external get: (t, string) => Js.Promise.t(SW_Client.t) = "get";
[@bs.send]
external matchAll_:
  (t, Js.Nullable.t(match_all_options)) => Js.Promise.t(array(SW_Client.t)) =
  "matchAll";

let matchAll = (self: t, ~includeUncontrolled: option(bool)=?, ~type_: option(string)=?, ()) => {
  let opts = if (includeUncontrolled->Belt.Option.isNone && type_->Belt.Option.isNone) {
...
[@bs.send]
external openWindow_: (t, string) => Js.Promise.t(Js.Nullable.t(SW_WindowClient.t)) =
  "openWindow";
