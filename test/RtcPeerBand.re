...
let updateDesc = desc => {
    ...
    let sdp =  desc##sdp->Js.Nullable.toOption->Option.getExn->updateBandwidthRestriction(maxBandwidth^);                  
    {
      "_type": Js.Nullable.return(type_),
      "sdp": Js.Nullable.return(sdp)
    }
}
...
let gotDescription2 = desc => {
    ...
    |> then_(() => {
        ...
        (if (maxBandwidth^ > 0) {
            getPc1()
            ->setRemoteDescription(updateDesc(desc))
        } else {
            getPc1()
...
let gotStream = s => {
    ...
    s->MediaStream.getTracks->Array.forEach(t => {
        pc1->RTCPeerConnection.addTrack(t, s) |> ignore;
    });
...
callButton->HTMLButtonElement.addEventListener_("click", _ => {
  ...
  navigator->Navigator.mediaDevices->Option.getExn
...
hangupButton->HTMLButtonElement.addEventListener_("click", _ => {
  ...
  localStream^ ->Option.getExn->MediaStream.getTracks->Array.forEach(t => {
...
bandwidthSelector->HTMLSelectElement.addEventListener_("change", _ => {
  ...
  let bandwidth = options->Array.get(bandwidthSelector->selectedIndex)
    ->Option.getExn->HTMLOptionElement.value;
  
  let sender = getPc1()->RTCPeerConnection.getSenders->Array.get(0)
    ->Option.getExn;
